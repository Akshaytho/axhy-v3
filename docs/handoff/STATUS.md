...
**Last updated:** 2026-05-15 (PR 1 blocked by repo-level baseline-migration-history gap; migration baseline recovery now required)
**Active phase:** **Migration-baseline recovery planning.** PR 1 (Layer 1 schema-only) is code-complete on `feat/layer-1-core-primitives` (3 commits: 215c208 + be954a2 + 853672c) but **cannot be locally verified because the repo's migration history is structurally incomplete** — the foundational baseline tables (Company, User, Membership, Site, Worker, Visit, LeaveRequest, OtpAttempt) are not captured in any migration. Surfaced 2026-05-15 12:30 during local-Postgres verification. Investigation note + ranked recovery options at `docs/plans/2026-05-15-migration-baseline-recovery.md`. Recommendation: reconstruct baseline migration in a separate PR ahead of PR 1's merge (Option 1). PR 1 holds unchanged until baseline recovery completes. PR 2 NOT started.
...
| **Current layer sequencing authority**               | `docs/plans/2026-05-15-implementation-kickoff-layer-1.md` | **Draft kickoff memo.** Authoritative for Layer 1 sequencing. Held pending migration-baseline recovery (see investigation note below).                                                                         |
| **Migration-baseline recovery investigation**        | `docs/plans/2026-05-15-migration-baseline-recovery.md`    | **Investigation note / proposal — pending founder pick.** 3 ranked options; recommendation = Option 1 (reconstruct baseline migration in a separate PR ahead of PR 1's merge). PR 1 holds until founder picks. |
