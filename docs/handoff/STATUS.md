...
**Last updated:** 2026-05-15 (baseline recovery merged into parent; PR 1 schema slice locally real-DB verified end-to-end; PR 2 next)
**Active phase:** **Layer 1 implementation — PR 1 code-complete + locally real-DB verified.** Migration baseline recovery merged into parent `feat/phase-c-wave-4b-chat-completion` at commit `1dbf951` (fast-forward). `feat/layer-1-core-primitives` rebased onto the updated parent; full migration chain (9 migrations) applies cleanly to a fresh local Postgres; all 9 expected Layer 1 schema objects present (4 tables: HRPod, Policy, Notification, Digest; 4 columns: Membership.podId, Worker.preferredLanguage, SupervisorDecision.originContext, SupervisorDecision.proposedDuringAbsence; 1 view: QueueItem). Smoke test confirmed FK + cascade behaviour. Migration is **NOT** yet applied to Railway prod — prod apply is a later supervised step, gated on `prisma migrate resolve --applied 20260507_phase_a_baseline_day3` + supervised window. Next slice: **PR 2 — audit-emit helpers for new AuditEvent kinds + real-DB integration tests around the new schema objects.** No new schema changes unless a real blocker appears.
...
| **Current layer sequencing authority**               | `docs/plans/2026-05-15-implementation-kickoff-layer-1.md` | **Draft kickoff memo.** Authoritative for Layer 1 sequencing. PR 1 (schema-only) locally real-DB verified 2026-05-15; PR 2 (audit-emit helpers + real-DB tests) next.                                         |
...
- **PR 2 — audit-emit helpers + real-DB integration tests.** Next slice on `feat/layer-1-core-primitives`. Scope: audit-emit helpers for the new AuditEvent kinds introduced by Layer 1; real-DB integration tests around the new schema objects. No new schema changes unless a real blocker appears; no Layer 2 surface work; no prod migration.
