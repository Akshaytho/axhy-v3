...
describe('chat → propose_termination', () => {
  it('voice → AI → DecisionCard → Apply → Worker.state = TERMINATION_PENDING', async () => {
    ...
    const chatRes = await app.inject({
      ...
      payload: {
        text: 'Fire Pradeep effective May 31 2026, performance issues',
      },
    });
    expect(chatRes.statusCode).toBe(200);
    const body = chatRes.json();
    expect(body.decisionCard?.toolName).toBe('propose_termination');
    ...
    const worker = await prismaRaw.worker.findUnique({ where: { id: workerId } });
    expect(worker?.state).toBe('TERMINATION_PENDING');

    const audit = await prismaRaw.auditEvent.findFirst({
      where: { companyId, kind: 'WORKER_TERMINATION_REQUESTED' },
    });
