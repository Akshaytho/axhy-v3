...
describe.skipIf(!process.env.REDIS_URL)(
  'POST /worker/visits/:visitId/submit — per-user rate limit',
  () => {
    it('returns 429 RATE_LIMITED + Retry-After header after exhausting the per-user budget', async () => {
      ...
      await getRedis().del(RedisKeys.rateLimit('worker:submit', workerUserId));

      const envKey = 'RATE_LIMIT_WORKER_SUBMIT_PER_MIN';
      const prev = process.env[envKey];
      process.env[envKey] = '2';
      try {
        // Use the existing otherVisitId (state=IN_PROGRESS) so each attempt
        // surfaces 409 WRONG_STATE before touching VisitPhoto rows — the rate
        // limit must fire AFTER auth+role but BEFORE business logic.
        for (let i = 0; i < 2; i++) {
...
describe.skipIf(!process.env.REDIS_URL)(
  'GET /worker/visits/:visitId/verify-status — per-user rate limit',
  () => {
    it('returns 429 RATE_LIMITED + Retry-After header after exhausting the per-user budget', async () => {
      const { getRedis } = await import('../src/lib/redis.js');
      const { RedisKeys } = await import('../src/lib/redis-keys.js');
      await getRedis().del(RedisKeys.rateLimit('worker:verify-status', workerUserId));

      const envKey = 'RATE_LIMIT_WORKER_VERIFY_STATUS_PER_MIN';
      const prev = process.env[envKey];
      process.env[envKey] = '2';
      try {
        for (let i = 0; i < 2; i++) {
