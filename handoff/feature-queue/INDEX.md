...
- **status:** `DONE` — friend's final CODE: APPROVED 2026-05-16 22:58 at HEAD `cbb7646`. Verbatim: "I re-reviewed the actual fix-up commit at HEAD cbb7646 on feat/f-007-notification-dispatcher, and both requested fixes are correctly in place ... CODE: APPROVED. Owner can push this branch and merge to main." Full F-007 commit chain: `41ee9e5` (round-1 scope) · `e6e605c` (round-2 v11 scope + tracker propagation) · `e0678d1` (round-2 v11 fix-up: expired_token + regenerate + status-wording) · `aee0784` (SCOPE_APPROVED transition) · `4c5d9d9` (code phase: composer + handler + outbox emits + migration + 11 tests = 120 cases) · `cbb7646` (round-2 fix-up: P1 worker-audience point-in-time filter + P2 acting_end integration tests + new point-in-time exclusion test = 123 cases). 20/20 files · 123/123 cases pass on fresh local Postgres 16 (109 prior baseline + 14 new F-007). **Merged to main at `79e38aa` on 2026-05-16.**
...
- **entities/routes/tables touched:**
  ...
  - **Failure policy — typed `failureReason` enum (v11 — 6 round-1 values, all actively emitted):** terminal-no-retry (3): `'no_active_subscription'` / `'recipient_inactive'` / `'recipient_removed'`. Transient-bounded-backoff-retry (3): `'rate_limited'` / `'provider_error'` / `'network_error'`. The enum is extensible — new values are added later only when F-011 (or a future provider) actually emits them from a real signal. Do NOT pre-register ghost values.
