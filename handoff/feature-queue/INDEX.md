...
- **status:** `SCOPE_DRAFT_PENDING_REVIEW (round 2)` — round-1 was CHANGES_REQUESTED for 3 drifts from closure spec §3.7 (siteRules from Site metadata; openItems split into openDecisions/upcomingShifts/7 days; dropped 4 spec fields generatedAt/outgoingSupervisorId/incomingSupervisorId/packageSizeBytes). After deep R6-as-real-user walkthrough + 4-rule-layer discussion + friend-caught 3 remaining mistakes (overclaim of "everything auto-switches"; conflate binding.handoffPackage with incoming LivingDoc updated; CalendarEntry filter still loose), scope revised 2026-05-16 to align EXACTLY to spec §3.7 + Decision 8 + owner-locked mechanism Z: acting cover writes `binding.handoffPackage` only; permanent rebind writes `binding.handoffPackage` AND copies outgoing's site-scoped siteRules into incoming `LivingDoc.siteRules` AND appends 1 handover-summary entry to incoming `LivingDoc.freeNotes`. 8 picks now locked; CalendarEntry filter locked at STRICT option (a) (explicit `payload.siteId` only; ambiguous skipped); 4 small mechanical Qs (createdBy enum fit, first-ever-binding case, size-cap truncation, idempotency rule-ID derivation); explicit non-claims section ("ownership-switch ≠ consumers done"; layer-1 company rules + layer-2 HR/pod rules stay LIVE-fetched, OUT of F-004; long-horizon planning is live-AI-query concern, not handoffPackage). Awaits owner + friend sign-off; no code lands until both sign off.
