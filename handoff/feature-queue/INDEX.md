...
- **status:** `SCOPE_DRAFT_PENDING_REVIEW (round 3)` — round-1 CHANGES_REQUESTED for 3 drifts (Site-metadata siteRules; openItems split into 3 buckets with 7-day window; dropped 4 spec fields). Round-2 CHANGES_REQUESTED for 2 more drifts caught by friend: (a) `recentComplaints` shape used `{id, text, severity, state, loggedAt}` instead of spec §3.7 line 307's `{id, kind, state, loggedAt, body}`; (b) `siteId` smuggled into top-level payload despite not being in spec §3.7's listed 8 fields; (c) 100KB truncation cited as spec-mandated without explicit line citation. **Round-3 revision 2026-05-16 evening:** payload shape aligned line-by-line to spec §3.7 (dropped `siteId` from top-level — binding row already carries it; adding it would be unauthorized denormalization); `recentComplaints` shape EXACTLY `{id, kind, state, loggedAt, body}` with severity dropped; mapping from `Complaint` row spelled out (`body = Complaint.text`, `loggedAt = Complaint.createdAt`, `state` derived from `resolvedAt`); **new Open Q5 surfaces a genuine schema gap** — Complaint model at schema.prisma:598-623 has no `kind` column, 3 options listed (constant `"site_complaint"` / map from `severity` rejected as misleading / add `Complaint.kind` column in separate slice and use constant as interim; default (a)); 100KB truncation cited explicitly to spec §3.7 Invariants subsection line 322 verbatim ("Package size capped at a Policy-configurable byte limit (default 100KB); truncation strategy: drop oldest complaints first, then activeWorkers field detail"); **new "Pre-decided product behavior" §0 section added at top per locked rule 27** answering the mandatory 5 questions (what product behavior is decided + citation; what UI surface proves it; LIVE vs SNAPSHOT bucketing of every data piece; redesign-audit of round-1 + round-2 + round-3 drifts; cost model verified against base+delta+live context model — composed ONCE per binding-create, NOT per chat message). 8 picks still locked unchanged in substance (picks 1, 3 are the wording fixes; Open Q3 + Open Q5 are the citation + schema-gap additions). Awaits owner + friend sign-off on round 3; no code lands until both sign off.
