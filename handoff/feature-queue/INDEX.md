...
- **status:** `AWAITING_APPROVAL` — code slice landed at `737c066` on branch `feat/f-003-cron-framework`. Scope LOCKED at `39b47b8` (Approach A approved + B rejected with permanent record in scope artifact §3). Pick 1 corrected pre-code at `74c1e9d` (dispatcher-tick piggyback, not OS-cron + HTTP — repo's actual existing pattern verified before code). Sweep emits `BINDING_ENDED_AUTO` audit per expired binding (no row mutation, point-in-time queries unaffected), one tx per row, audit-existence idempotency, dispatcher-piggyback every 5 min. 18/18 test files green · 92/92 cases pass on fresh local Postgres 16 (84 baseline + 8 new F-003 cases). Awaits friend's file-grounded approval.
