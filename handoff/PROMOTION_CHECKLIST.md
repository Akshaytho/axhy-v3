...
5. ☐ **`STATUS.md` updated** — move the new spec from "Draft" to "Approved + Active"; remove old deferred items that are now answered by it.
6. ☐ **`NEXT_SESSION.md` updated** — reflect the new authority status; remove "Draft pending approval" qualifiers for this spec.
7. ☐ **`ROADMAP.md` updated** if the promotion advances the build phase (e.g., Layer 1 unblocks Layer 2 planning).
8. ☐ **`done/<date>-<phase>.md` archive entry added** if this promotion closes a phase.
9. ☐ **Memory pointer wording updated** in `MEMORY_V3.md` if the previous wording said "Draft pending approval" for this spec. Remove that qualifier.
10. ☐ **Commit message documents the promotion** — include all checklist items in the commit body so the git history records the full state change.

## Verification after promotion

- Grep the repo for "Draft" references to the now-Active spec → update each one to "Active."
- Check `STATUS.md`'s "Authority Snapshot" + "Approved vs Draft" sections are consistent.
...
- `STATUS.md` would become inconsistent after a partial promotion → finish the full checklist before merging.
