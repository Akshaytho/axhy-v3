...
1. **Phase 2d part 2 — remaining domain reads** (same `withTenantRead(prisma, auth.companyId, async (tx) => tx.…)` transform; use `async` closures because of nested-relation selects):
   - `routes/admin-workers.ts`: LIST read `:106` is DONE (wrapped). The DETAIL handler GET /admin/workers/:id still needs wrapping: `:167` worker.findFirst (nested user→memberships select) + `:201`/`:279` assignment.count — and it calls `getHrSiteIds(prisma, …)` at `:200`. Wrap the whole HR branch in ONE `withTenantRead(prisma, auth.companyId, async (tx) => {…})` and pass `tx` into `getHrSiteIds(tx, …)` + the counts (getHrSiteIds already accepts a `Prisma.TransactionClient`).
   - `routes/leave-requests.ts`: `:202` leaveRequest.findFirst, `:212` assignment.count, `:495` leaveRequest.findMany, `:535` leaveRequest.findFirst, `:566` assignment.count. Add `withTenantRead` to the import (line 35 group). NOTE: check whether these handlers also call `getHrSiteIds`/`resolveWorkerFromAuth`/other helpers — pass them a GUC-bound tx too.
   ...
   - **⚠ HELPER/SERVICE AUDIT (discovered this session — bigger than the route inventory):** helpers/services that take a `db: PrismaClient | Prisma.TransactionClient` and read tenant tables (confirmed: `middleware/hr-site-scope.ts getHrSiteIds` reads Site) return 0 rows under `axhy_app` if called with base `prisma`. They already accept a tx, so the fix is to pass a GUC-bound tx at every call site. Before the switch, grep services/lib/middleware for functions that take a client param + do `db.<tenantModel>.` and verify each call site passes a `tx` (e.g. `getHrSiteIds`, assignment/complaint/leave services). This is the main remaining unknown surface.
