...
27. **Pre-decided product behavior is an input, not a topic (Akshay rule, 2026-05-16, locked after F-004 round-2 drift).** Before any new backend design, restate the already-decided product behavior in simple English and verify the draft matches it. Owner-locked product behaviors are constraints the design satisfies — they are NOT problems for the design to re-solve.
    ...
    - **Locked product behaviors (the running list — additions land here as owner decides them):**
      - **Base + Delta + Live context model (locked 2026-05-16, owner directive after F-004 round-2 review).** The supervisor chat context is composed in three layers:
      ...
      - **HandoffPackage = focused snapshot of outgoing supervisor's JUDGEMENT (locked closure spec §3.7 + Decision 8).** It captures what the outgoing person decided about this site at this moment (rules, recent complaints, active workers, open items in next 14 days). It is NOT "everything we know about the site" — long-horizon planning queries stay LIVE via the chat tool path. Truncation policy locked at spec §3.7 Invariants line 322 (100KB cap, drop oldest complaints first then activeWorkers field detail).
      - **4 rule layers (locked closure spec §3.7 + audit Ravi Month 9b).** Layer 1 company-permanent rules (HRUpdate / HRUpdateRule) stay LIVE-fetched, not in handoff. Layer 2 HR/pod rules stay LIVE-fetched, not in handoff. Layer 3 site-specific supervisor rules TRANSFER on permanent rebind (mechanism Z — copy to incoming LivingDoc.siteRules + summary in freeNotes). Layer 4 personal supervisor working notes STAY with original supervisor.
    - **Concrete example (F-004 saga, the rule's origin):** the F-004 round-1 draft sourced `siteRules` from `Site` metadata (name/address/state/workdays) instead of the outgoing supervisor's `LivingDoc.siteRules` — because "site rules" in plain English was re-shaped into "static site attributes" in backend thinking. Round-2 still drifted: `recentComplaints` shape was redesigned to `{id, text, severity, state, loggedAt}` instead of the spec's `{id, kind, state, loggedAt, body}`, and a `siteId` field was smuggled into the top-level payload (not in spec's listed 8 fields). Both rounds failed because the design was reasoning from backend-shape-of-data, not from the already-decided product shape. The base+delta+live model — owner's pre-existing product decision — was also re-debated in the round-2 discussion when it should have been pinned at the top of the artifact as a locked input.
