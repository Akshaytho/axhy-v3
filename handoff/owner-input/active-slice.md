...
| **Status**                              | `CHANGES_REQUESTED` (round 2) — friend's second production-grade pass found 3 remaining issues at the orchestration layer. Core writer is much safer; the route plumbing around it still has corruption holes. Remediation plan in `pending-approvals.md`.   |
