...
| **Status**             | `AWAITING_APPROVAL (round 2 v11 — code phase)` 2026-05-16. Real-DB sweep green: **20/20 files · 120/120 cases pass** on fresh local Postgres 16 (109 prior baseline + 11 new F-007 cases). Awaits friend's file-grounded round-2 code review.                                                                            |
...
| **Tests status**       | GREEN — 20/20 files · 120/120 cases pass on fresh local Postgres 16 after applying migration `20260520_f007_notification_supervisor_change_idempotency`. New test file `apps/backend/test/notification-supervisor-change.test.ts` lands with the code commit; target ~120 cases total (109 prior baseline + ~11 new).    |
