...
| **Status**             | `AWAITING_APPROVAL (round 2 v11 — code phase, round-2 fix-up)` 2026-05-16. Real-DB sweep green: **20/20 files · 123/123 cases pass** on fresh local Postgres 16 (109 prior baseline + 14 new F-007 cases). Friend's round-1 code review at HEAD `4c5d9d9` surfaced 2 findings (P1 worker-audience point-in-time filter missing; P2 acting_end branch untested); both fixed in this commit. Awaits friend's re-review. |
| **Branch**             | `feat/f-007-notification-dispatcher` — forked from main `04c5e59` 2026-05-16. v11 scope + tracker propagation landed at `e6e605c`; no code yet (awaits owner + friend round-2 v11 SCOPE: APPROVED on the committed propagation before code begins).                                                                                                                                                                   |
| **Last landed commit** | `e6e605c` — `docs(handoff): F-007 round 2 v11 — Notification persistence + audience resolution (OneSignal direction; immutable rows; collapsed eligibility rule)`                                                                                                                                                                                                                                                     |
...
| **Tests status**       | GREEN — 20/20 files · 123/123 cases pass on fresh local Postgres 16 after applying migration `20260520_f007_notification_supervisor_change_idempotency`. New test file `apps/backend/test/notification-supervisor-change.test.ts` covers 14 F-007 cases (round-1 11 + round-2 fix-up tests 12 acting_end / 13 acting_end-no-permanent-skip / 14 point-in-time worker-audience exclusion).                             |
