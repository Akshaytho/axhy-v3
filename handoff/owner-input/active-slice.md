...
| **Status**             | `CODE_AWAITING_APPROVAL` 2026-05-17 01:40 (after friend's CODE-phase P1 fix-up). Friend caught one P1: `OneSignal.initialize(appId)` was never called at runtime — the Expo plugin only wires native build capabilities, not the JS SDK. Fix-up landed: new `initializeOneSignal()` export in identity-lifecycle.ts (idempotent, web/no-App-ID-guarded) + wired into `app/_layout.tsx` `useEffect` + 3 new unit cases. 45/45 tests green. Manual smoke gained a new Scenario 0 ("confirm SDK initializes on app boot"). Awaiting friend's `CODE: APPROVED`. |
...
| **Tests status**       | GREEN — 18 F-006a unit cases passing inside a 45/45 total via `pnpm --filter @axhy/mobile test`. Files: `apps/mobile/lib/identity-lifecycle.test.ts` (13 cases — 10 lifecycle + 3 init-fix-up) + `apps/mobile/components/PushPermissionPrompt.test.ts` (5 cases on the pure `buildPromptOutcomeRunner` factory).                                                                                                                                                                                                                                            |
