...
| **Status**             | `CODE_AWAITING_APPROVAL` 2026-05-17 01:55 (after friend's CODE-phase P1 round-2 / CHANGES_REQUESTED). Friend caught the parent-child useEffect race: `_layout.tsx`'s warm-up `useEffect` runs AFTER `app/index.tsx`'s cold-start `useEffect`, so the authed-relaunch path could still hit `OneSignal.login` before init. Round-2 fix moves the init-before-use guarantee INTO `_resolveOneSignal()` itself — every lifecycle path goes through that chokepoint, so init is awaited regardless of React effect order. Root-layout `useEffect` stays as a warm-up. 2 new ordering test cases prove init-before-login on cold-start + identified-login paths. 47/47 tests green. Awaiting friend's `CODE: APPROVED`. |
...
| **Tests status**       | GREEN — 20 F-006a unit cases passing inside a 47/47 total via `pnpm --filter @axhy/mobile test`. Files: `apps/mobile/lib/identity-lifecycle.test.ts` (15 cases — 10 lifecycle + 3 init + 2 ordering) + `apps/mobile/components/PushPermissionPrompt.test.ts` (5 cases on the pure `buildPromptOutcomeRunner` factory).                                                                                                                                                                                                                                                                                                                                                                                            |
