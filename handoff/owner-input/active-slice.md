...
| **Status**             | `SCOPE_APPROVED (round 2 v11)` 2026-05-16. Friend's APPROVED verdict at HEAD `e0678d1` 22:14 verbatim: "SCOPE: APPROVED (round 2 v11). You can let the owner push this branch, and then begin code on the same branch per the scoped file list. Stop again at AWAITING_APPROVAL after the implementation test sweep is green." Code now in flight on the same branch per the v11 in-scope code surface. |
...
| **Tests status**       | WIP — code in flight after SCOPE_APPROVED at HEAD `e0678d1` 22:14. New test file `apps/backend/test/notification-supervisor-change.test.ts` lands with the code commit; target ~120 cases total (109 prior baseline + ~11 new).                                                                                                                                                                         |
