...
| **Status**             | `AWAITING_APPROVAL` 2026-05-16 — code slice complete on `feat/f-004-handoff-package-composer`. Full real-DB sweep **19/19 files · 106/106 cases pass** (95 baseline + 11 new F-004 cases). 3 new files (`packages/shared-schema/src/zod/handoff-package.ts`, `apps/backend/src/lib/handoff-package-composer.ts`, `apps/backend/src/lib/handoff-package-writer.ts`). 2 wiring edits (`apps/backend/src/lib/site-supervisor-binding.ts` — new `recordHandoffPackageGenerated` typed helper + `reassignPermanentBinding` extended with compose-before-create + writer-after; `packages/shared-schema/src/zod/audit-payloads.ts` + `index.ts` — new `HandoffPackageGeneratedPayloadSchema` + barrel export). Round-4 v4 scope artifact requirements implemented verbatim: 9 canonical fields with `schemaVersion: 1` first; mechanism Z permanent-rebind LivingDoc copy; Q2 = (b) zero LivingDoc writes on first-ever binding; STRICT CalendarEntry filter; 4 reads parallelised via `Promise.all`; truncation per spec §3.7 line 322; deterministic uuid-v5-shaped rule IDs for idempotency; clientPreferences NOT transferred (Material #2 = β). All 3 panel code-stage notes addressed inline. |
...
## 8 picks locked in the round-4 v4 scope artifact
...
1. **Payload shape** — **matches spec §3.7's amended 9-field set** (post-amendment 2026-05-16 — `schemaVersion` added as canonical 9th field per Material #1 = γ), with one interim mapping noted in pick 3 + Open Q5 for `Complaint.kind`. Zod `HandoffPackagePayloadSchema`, in this order: **`schemaVersion: 1`** (literal-typed integer; placed first; Zod rejects any other value), `generatedAt`, `outgoingSupervisorId?` (NULL on first-ever binding), `incomingSupervisorId`, `siteRules: string[]`, `recentComplaints[]`, `activeWorkers[]`, `openItems[]`, `packageSizeBytes`. **No `siteId` on top-level**. Honest framing: field-list match is exact (9 fields per amended spec); `kind` content is interim default until a dedicated `Complaint.kind` schema column exists. Consumers MUST inspect `schemaVersion` first and fail closed on unknown versions per the new spec §3.7 invariant.
