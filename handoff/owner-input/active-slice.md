...
| **Status**             | `SCOPE_APPROVED (round 2 v6)` 2026-05-17 00:56. Friend's CODE-phase directive verbatim: "Proceed to code on feat/f-006a-onesignal-identity-lifecycle and stop at AWAITING_APPROVAL after unit tests + manual smoke are documented." Code phase in progress on the branch.                                   |
...
| **Tests status**       | WIP — 15 unit cases planned across 2 files (`apps/mobile/lib/identity-lifecycle.test.ts` 10 cases + `apps/mobile/components/PushPermissionPrompt.test.tsx` 5 cases). Run via `pnpm --filter mobile test`.                                                                                                   |
