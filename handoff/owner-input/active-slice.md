...
| **Status**             | `CODE_AWAITING_APPROVAL` 2026-05-17 01:25. Code phase complete on `feat/f-006a-onesignal-identity-lifecycle` (no push, no merge). 42/42 unit tests green (incl. the 15 F-006a-new cases). Manual smoke plan documented at `handoff/owner-input/f-006a-manual-smoke.md` — owner runs on real device + OneSignal sandbox before merge. Awaiting friend's `CODE: APPROVED`.                                                            |
...
| **Tests status**       | GREEN — 15 F-006a unit cases passing inside a 42/42 total via `pnpm --filter @axhy/mobile test`. Files: `apps/mobile/lib/identity-lifecycle.test.ts` (10 cases) + `apps/mobile/components/PushPermissionPrompt.test.ts` (5 cases on the pure `buildPromptOutcomeRunner` factory — vitest include glob is `components/**/*.test.ts`, so the .ts extension matters; the React component itself is a thin wrapper around the factory). |
