...
| **Status**             | `SCOPE_DRAFT_PENDING_REVIEW (round 4 v3)` — rounds 1+2+3 had shape/honesty drifts; round-4 v1 added Postgres+pgvector 4-bucket lock; round-4 v2 fixed 3 scope-truth contradictions; **round-4 v3 (this revision) locks Open Q2 at (b)** — owner picked 2026-05-16: NO handover-summary entry on first-ever binding (no outgoing supervisor exists; `generatedAt` + `outgoingSupervisorId: null` already give chronology; do not add noise to `freeNotes`). Locked behavior on first-ever binding (permanent path): zero LivingDoc writes; only the binding row + `binding.handoffPackage` JSON state change; 1× `HANDOFF_PACKAGE_GENERATED` audit fires; zero `LIVING_DOC_RULE_ADDED` audits. **Final gate before code:** real panel pass on round-4 v3 scope (owner directive: do not defer panel discipline to F-005; F-004 is still in scope phase). Panel critique companion file lands at `handoff/feature-queue/scopes/F-004-panel-review.md`. If panel pass does not surface a new material issue → SCOPE: APPROVED → code begins. 8 picks substance unchanged. |
...
8. **Mechanism Z (owner-locked) — two write paths by binding kind:**
   ...
   - **First-ever-binding case** (no outgoing) — **Q2 LOCKED at (b) 2026-05-16:** zero LivingDoc writes; only the binding row + `binding.handoffPackage` JSON state change. No `freeNotes` summary entry.

## 5 small open mechanical questions — Q2 LOCKED at (b) 2026-05-16; Q1/Q3/Q4/Q5 have recommended defaults

**Q2 RESOLVED 2026-05-16 — owner picked (b):** for first-ever binding, write NO handover-summary entry. Reason (owner verbatim): "there is no outgoing supervisor, so this is not really a handover-from-someone case. `generatedAt` + `outgoingSupervisorId: null` on the package already give enough chronology. Do not add noise into `freeNotes`." Locked behavior on first-ever binding (permanent path): zero LivingDoc writes; only the binding row + `binding.handoffPackage` JSON state change; 1× `HANDOFF_PACKAGE_GENERATED` audit fires; zero `LIVING_DOC_RULE_ADDED` audits.

Q1 — `LivingDocRule.createdBy` enum doesn't have "system_handover". Default: use `'supervisor'`; provenance preserved in `source.pattern`. **Q2 — LOCKED at (b)** (see above). Q3 — 100KB size-cap truncation strategy is **NOT invented by F-004 — it is spec §3.7 Invariants subsection, line 322, locked verbatim**. Code-stage detail. Q4 — idempotency on replay via deterministic rule IDs (uuid-v5). Q5 — **`kind` field in ComplaintSummary is a genuine schema gap** (Complaint model has no `kind` column). Default: **(a)** constant `kind: "site_complaint"` unless owner wants future categorization.
...
## Decision needed (owner + friend, on round-4 v3 — Q2 locked at (b), panel pass pending)

**Q2 LOCKED at (b) 2026-05-16** (no first-ever-binding summary entry). Remaining gate: a real panel pass on the scope before code starts (owner directive 2026-05-16: "do not defer panel discipline to F-005; F-004 is still in scope phase, so this is the cheapest and safest time to do it"). Panel critique companion file landing in the same commit chain at `handoff/feature-queue/scopes/F-004-panel-review.md`. If the panel pass does not surface a new material issue:

- `SCOPE: APPROVED (round 4 v3)` → I begin code immediately on `feat/f-004-handoff-package-composer`. Stop at `AWAITING_APPROVAL` after the new test sweep is green.
- `SCOPE: CHANGES_REQUESTED` → name the pick / Open Q / panel finding to address; I revise + re-surface.
- `HOLD` → F-004 pauses; surface a different next slice (F-005, F-006, or F-007).
