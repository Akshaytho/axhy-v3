...
- **Open Q2 reframed as a new product choice (not pre-locked).** What spec locks: empty siteRules, no copy-loop iterations (mechanically clear). What spec does NOT lock: whether to write a summary entry at all on first-ever binding, and what wording. 3 options listed: (a) Write "First binding for `<siteName>` on `<date>` — no prior supervisor context"; (b) Write NO summary entry; (c) Owner-supplied wording. Recommended (a) — but owner explicitly picks.
...
**5 small mechanical Open Qs** (Q2 is a true open product choice in round-4; others have recommended defaults):
...
- **Q2 (REFRAMED IN ROUND-4 AS A NEW PRODUCT CHOICE):** what to do for first-ever binding's summary entry. Spec locks: empty siteRules, no copy iterations. Spec does NOT lock: whether to write any summary entry at all, and if so what text. **3 options — owner picks:** (a) Write "First binding for `<siteName>` on `<date>` — no prior supervisor context"; (b) Write NO summary entry — nothing to hand over; empty entry is noise; (c) Owner-supplied wording. Recommended (a) — but NOT a default-with-fallback; owner explicitly picks.
...
**Test plan:** 8 real-DB integration cases — acting binding / permanent reassign / first-ever (with owner-picked Q2 option) / empty-state / cross-tenant / STRICT calendar filter (with & without payload.siteId) / idempotent replay.
