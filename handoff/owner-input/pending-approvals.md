...
> [P1] The no-op-on-init-failure contract is still not actually enforced. In apps/mobile/lib/identity-lifecycle.ts initializeOneSignal() logs and returns when OneSignal.initialize is missing or throws, but \_resolveOneSignal() still proceeds to import the SDK and return a live handle. That means onIdentifiedLogin, onAppLogout, onColdStartReady, and the prompt path can still call login / logout / requestPermission against an uninitialized SDK after the exact failure path the comments say should "no-op". This reopens the production bug under partial-SDK / runtime-init-failure conditions. \_resolveOneSignal() should only return a handle if initialization actually succeeded.
>
> [P2] The new init latch is not concurrency-safe, so startup can still double-initialize. In apps/mobile/lib/identity-lifecycle.ts, oneSignalInitialized is only a boolean checked before the async import. If the warm-up in apps/mobile/app/\_layout.tsx and the first chokepoint call in \_resolveOneSignal() race, both callers can observe false, both await the import, and both call OneSignal.initialize(appId) before either flips the latch. The new tests prove sequential idempotence and init-before-login ordering, but not concurrent warm-up + first-use behavior. This should be a shared promise latch, not a bare boolean.

**Round-3 fix-up (this commit):**

- **P1 — init success gates `_resolveOneSignal()`.** `initializeOneSignal()` now returns `Promise<boolean>`. Promise resolves to `true` ONLY when `OneSignal.initialize(appId)` actually ran; every failure path (`shouldCallOneSignal()` false / missing App ID / SDK module load fail / missing `initialize` function / `initialize` throws) resolves to `false`. `_resolveOneSignal()` checks `if (!initOk) return null` and short-circuits BEFORE the second `await import('react-native-onesignal')`. Downstream `login` / `logout` / `requestPermission` never run against an un-initialized SDK.
- **P2 — bare-boolean latch replaced with `Promise<boolean>` latch.** Former `let oneSignalInitialized = false` had a TOCTOU race; two concurrent callers could both observe `false`, both `await import(...)`, both call `OneSignal.initialize`. Fix: `let initPromise: Promise<boolean> | null = null` — first caller assigns the in-flight promise; concurrent callers return the SAME promise. Init failure is session-sticky (no retry storm).
- **2 NEW unit cases (16 + 17):** Case 16 (P1 proof) — `OneSignal.initialize` throws; `OneSignal.login` NEVER called, `setTokens` still ran. Case 17 (P2 proof) — `Promise.all` of 4 concurrent `initializeOneSignal()` calls produces EXACTLY ONE `OneSignal.initialize` invocation.

**Status update 2026-05-17 03:00:** Round-3 fix-up complete on `feat/f-006a-onesignal-identity-lifecycle` (not pushed). Unit-test sweep **49/49** green (22 F-006a-new cases: identity-lifecycle 17 + PushPermissionPrompt 5). Awaiting friend's `CODE: APPROVED`.

**Earlier status (superseded by round-3):** 2026-05-17 01:55 = chokepoint pattern + 47/47; 2026-05-17 01:40 = root-layout warm-up + 45/45; 2026-05-17 01:25 = original code phase + 42/42.

**Friend's CODE-phase P1 round-2 verbatim 2026-05-17 01:51 (CHANGES_REQUESTED):** "OneSignal initialization is still racy on the cold-start authed path. You added initializeOneSignal() in a parent useEffect in apps/mobile/app/\_layout.tsx (line 31), but the authed cold-start relink is triggered from a child useEffect in apps/mobile/app/index.tsx (line 26) and then calls OneSignal.login(...) via apps/mobile/lib/identity-lifecycle.ts (line 261). In React, child useEffects can run before parent useEffects on mount, so an already-signed-in user can still hit onColdStartReady() before \_layout has initialized the SDK. That leaves the exact production gap this fix was meant to close on the authed relaunch path. The new tests only prove initializeOneSignal() is idempotent in isolation; they do not prove initialize happens before login/requestPermission/logout in the mounted app flow. I'd fix this by making initialization part of the lifecycle boundary itself, not just the root layout side effect: either \_resolveOneSignal() should ensure the SDK is initialized before returning login/logout/requestPermission capability, or onIdentifiedLogin / onColdStartReady / the prompt runner should explicitly await initialization before first SDK use. ... Verdict: CODE: CHANGES_REQUESTED. This is close, but the init-before-login guarantee still needs to be made real on the cold-start path."

**Round-2 fix-up (this commit):**

- `_resolveOneSignal()` in `apps/mobile/lib/identity-lifecycle.ts` now `await`s `initializeOneSignal()` BEFORE returning the SDK handle. Since every lifecycle path (`onIdentifiedLogin` / `onAppLogout` / `onColdStartReady` / the prompt's default `requestPermission`) goes through `_resolveOneSignal()`, init-before-call is guaranteed regardless of useEffect ordering. The root-layout `useEffect` stays in place but is now demoted to a WARM-UP (pre-pays init latency so the first lifecycle call is fast). Both calls are safe because `initializeOneSignal()` is idempotent via the module-level latch.
- 2 NEW ordering test cases (cases 14 + 15) in `apps/mobile/lib/identity-lifecycle.test.ts` — they call `onColdStartReady()` / `onIdentifiedLogin()` WITHOUT a prior `initializeOneSignal()` invocation (simulating the cold-start race where `_layout.tsx`'s warm-up hasn't fired yet) and use a shared `callOrder: string[]` array to assert `OneSignal.initialize` is called BEFORE `OneSignal.login`. These two cases directly prove init-before-login in the actual mounted-app code path, not just in isolation.
- JSDoc in `_layout.tsx` + `initializeOneSignal` + `_resolveOneSignal` updated to record the chokepoint pattern + the React mount-order rationale so future readers don't re-introduce the race.

**Status update 2026-05-17 01:55:** Round-2 fix-up complete on `feat/f-006a-onesignal-identity-lifecycle` (not yet pushed; no merge). Unit-test sweep **47/47** green via `pnpm --filter @axhy/mobile test` (the 20 F-006a-new cases land inside that total: `lib/identity-lifecycle.test.ts` 15 cases — 10 lifecycle + 3 init + 2 ordering — + `components/PushPermissionPrompt.test.ts` 5 cases). Awaiting friend's `CODE: APPROVED`.

**Earlier status (2026-05-17 01:40 — superseded by round-2 fix-up above):** added `initializeOneSignal()` + wired into `_layout.tsx` `useEffect`; 45/45 tests; friend caught the parent-child useEffect race that this round-2 fix resolves.

**Earlier status (2026-05-17 01:25 — superseded):** original code phase complete; 42/42 test sweep; manual smoke documented.

**Friend's CODE-phase P1 review 2026-05-17 01:35 verbatim:** "[P1] OneSignal is never initialized before the new lifecycle starts calling login/logout/requestPermission. The code now dynamically resolves the SDK and immediately uses login() / logout() in apps/mobile/lib/identity-lifecycle.ts (line 119), apps/mobile/lib/identity-lifecycle.ts (line 174), and apps/mobile/lib/identity-lifecycle.ts (line 225), and it calls Notifications.requestPermission() in apps/mobile/components/PushPermissionPrompt.tsx (line 77). The config migration in apps/mobile/app.config.ts (line 19) only exposes the App ID/build plugin wiring; it does not initialize the JS SDK at runtime. I could not find any OneSignal.initialize(...) call anywhere under apps/mobile. That means the happy-path tests can still pass with mocks while the real-device smoke flow silently no-ops or fails once it reaches the first identified login / permission prompt. This needs one explicit runtime initialization path before any login/requestPermission/logout call is treated as production-ready. ... Once the SDK initialization is added and the real-device smoke confirms the login + permission + external_id link path, I'd expect this to be very close to CODE: APPROVED."

**Fix-up commit on `feat/f-006a-onesignal-identity-lifecycle` (forthcoming):**

- NEW `initializeOneSignal()` export in `apps/mobile/lib/identity-lifecycle.ts` — dynamic `import('react-native-onesignal')`, calls `OneSignal.initialize(appId)` exactly once per cold-start, guarded by `shouldCallOneSignal()` (web / no-App-ID → no-op with warning), idempotent via module-level latch (`oneSignalInitialized` flag protects against React StrictMode + fast refresh + accidental re-mount), errors swallowed with warning log so a failed init never breaks auth.
- WIRED in `apps/mobile/app/_layout.tsx` — `useEffect(() => { initializeOneSignal(); }, [])` on the root layout component fires init once on mount, before any route renders (since `app/index.tsx` and downstream screens mount inside `<Slot />`).
- NEW 3 unit cases inside `apps/mobile/lib/identity-lifecycle.test.ts` (cases 11–13): idempotent across repeated calls (case 11 — 3 calls, only one `OneSignal.initialize` invocation); web no-op (case 12); no-App-ID no-op (case 13). Total identity-lifecycle cases: 13; total F-006a-new: 18; sweep total: 45/45.
- NEW `_resetOneSignalInitializedForTests()` test-only escape hatch so the module-level latch can be reset between cases (called from `beforeEach`).
- Manual smoke doc gains a **new Scenario 0** ("Confirm SDK initializes on app boot") that the owner runs FIRST — confirms an anonymous OneSignal subscription appears on the device before any OTP login, proving init actually fired on the real-device path the mocks can't reach.

**Status update 2026-05-17 01:40:** Code phase + P1 fix-up complete on `feat/f-006a-onesignal-identity-lifecycle` (not yet pushed; no merge per blanket no-push-without-review feedback rule). Stop at AWAITING_APPROVAL per friend's directive. Unit-test sweep **45/45** green via `pnpm --filter @axhy/mobile test` (the 18 F-006a-new cases land inside that total: `lib/identity-lifecycle.test.ts` 13 cases — 10 lifecycle + 3 init — + `components/PushPermissionPrompt.test.ts` 5 cases on the pure `buildPromptOutcomeRunner` factory). Mobile typecheck: only pre-existing `localStorage` errors in `lib/auth-store.ts` (web-fallback shim sans DOM lib; not introduced by F-006a). Manual smoke plan: `handoff/owner-input/f-006a-manual-smoke.md` (5 scenarios now: Scenario 0 boot-init + Scenarios 1-4 + 3 negative paths; requires real device + OneSignal sandbox project — flagged for owner to run before merge).

**Earlier status (2026-05-17 01:25 — superseded by the P1 fix-up above):** Code phase complete; 42/42 test sweep; manual smoke documented.
...
**Round 2 v11 history (round 1 → 2-v2 → v3 → v4 → v5 → v6 → v7 RESET → v8 → v9 → v10 → v11):** see plan file `/Users/thotaakshay/.claude/plans/yes-you-can-start-ancient-yao.md` for full v1 → v11 review history. Key transitions: v7 dropped supervisor write-time coalescing entirely (immutable rows; store truth first, group later — Stripe/AWS/GitHub/Slack/Novu pattern); v8 redirected F-011 from FCM/Expo to OneSignal as the managed push provider; v9-v11 closed reachability + deactivation/logout lifecycle gaps. Comprehensive 11-voice production-readiness panel test 2026-05-16 21:35 — **friend's verdict on the v11 DRAFT 21:36 verbatim:** "REVIEWED. Good panel pass. Fold all 5 material findings into v11. Defer neither. This panel pass improved the plan. It did not expose a new architecture reset." All 5 folded into the plan. **The current commit (HEAD `e6e605c`) is the scope-artifact + tracker propagation materialising the panel-tested v11 plan** — it has NOT yet received friend's separate file-grounded review and is the subject of this AWAITING_APPROVAL entry. No code lands until owner + friend sign off on THIS commit.
